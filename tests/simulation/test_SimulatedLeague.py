...
class TestDraft:
    ...
    def test_run_draft_broadcasts_picks(self, mock_gen_schedule, mock_init_teams,
        ...
        mock_teams = [Mock() for _ in range(2)]
        for team in mock_teams:
            team.config = Mock()
            team.get_draft_recommendation.return_value = Mock(id=1, name="Test", position="QB")
...
class TestResults:
    ...
    def test_get_draft_helper_results_returns_tuple(self, mock_gen_schedule, mock_init_teams,
        ...
        league.week_results = [mock_week] * 3
...
class TestJSONLoading:
    ...
    def test_parse_players_json_all_positions(self, tmp_path):
        ...
        assert 0 in players
        assert 10000 in players
        assert 20000 in players
        assert 30000 in players
        assert 40000 in players
        assert 50000 in players
...
class TestExplicitConstructionSnapshotGuard:
    ...
    cache here is deliberately non-empty (TD3). Unlike TestSelfLoadRefusesMissingActuals,
    _initialize_teams is NOT patched out — the new guard lives inside it.
    """

    def test_missing_week_18_raises_before_shared_data(self, tmp_path):
        weeks_folder = tmp_path / "weeks"
        weeks_folder.mkdir()
