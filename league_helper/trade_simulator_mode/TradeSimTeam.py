...
class TradeSimTeam:
    ...
    def score_team(self) -> float:
        ...
        for player in self.team:
            if self.isOpponent:
                scored_player = self.player_manager.score_player(player, player_rating=False, team_quality=False, consistency=False)
            else:
                scored_player = self.player_manager.score_player(player)
