# Handoff Package: Feature 04 (accuracy_sim_logging)
...
**My Feature Assignment:** Feature 04: accuracy_sim_logging
...
logger = setup_logger(
    name="accuracy_simulation",     # Script name (becomes folder name)
...
# Result: logs/accuracy_simulation/accuracy_simulation-{timestamp}.log with 500-line rotation
```

**Integration Contracts (must follow):**
1. **Logger name = folder name:** Use consistent name "accuracy_simulation" (not variations)
...
- **Feature 04: accuracy_sim_logging** (Secondary-C - **THIS IS ME**)
...
1. **S2.P1.I1 - Discovery:** Research accuracy simulation script and modules, create RESEARCH_NOTES.md, draft spec.md
...
- `feature_04_accuracy_sim_logging/HANDOFF.md` (feature-specific context - see below)
- `feature_01_core_logging_infrastructure/spec.md` (Group 1 dependency)

**Coordination:**
- Update `parallel_work/coordination/agent_checkpoints/secondary_c_checkpoint.md` every 15 minutes
- Check `parallel_work/coordination/inboxes/from_primary/` for messages from Primary
- Update `feature_04_accuracy_sim_logging/STATUS` at phase transitions
- Escalate to `parallel_work/coordination/inboxes/from_secondary_c/` if blocked
...
4. **Feature 04: accuracy_sim_logging** ← **I'M WORKING ON THIS**
...
**Script Entry Point:**
- `run_accuracy_simulation.py` - Direct entry point, already has --log-level flag (good precedent for --enable-log-file)

**Main Module:**
- `simulation/accuracy/AccuracySimulationManager.py` - Main simulation manager, logging setup

**Core Modules:**
- `simulation/accuracy/AccuracyCalculator.py` - Accuracy calculation logic
- `simulation/accuracy/AccuracyResultsManager.py` - Results management
- `simulation/accuracy/ParallelAccuracyRunner.py` - Parallel execution

**Tests:**
- `tests/simulation/accuracy/` - May need updates if log assertions change
...
- ✅ Significant outcomes (e.g., "Simulation complete: 10 leagues")
...
4. **Update Agent Status:**
   - File: `feature_04_accuracy_sim_logging/README.md`
   - Update "Agent Status" section with current guide, step, timestamp

5. **Initialize coordination:**
   - Create `parallel_work/coordination/agent_checkpoints/secondary_c_checkpoint.md`
   - Initial status: STARTUP, feature: 04, timestamp

6. **Begin S2.P1.I1 (Discovery):**
   - Research run_accuracy_simulation.py (already has --log-level precedent)
   - Research simulation/accuracy/ modules
   - Identify all logger.debug/info calls in accuracy simulation scope
...
**Every 15 minutes:**
- Update `agent_checkpoints/secondary_c_checkpoint.md` with current status
- Check `inboxes/from_primary/` for messages
- Report progress or blockers

**When blocked:**
- Create escalation file in `inboxes/from_secondary_c/`
...
- I'm working in parallel with 5 other secondary agents (Features 02-03, 05-07)
...
**Good luck with Feature 04! If you have questions, escalate to Primary via inbox.**
