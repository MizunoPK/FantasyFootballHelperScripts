# STAGE 4: Epic Testing Strategy Guide
...
1. ⚠️ Stage 3 (user approval) MUST be complete before Stage 4
   - Cannot update testing strategy without approved plan

2. ⚠️ epic_smoke_test_plan.md is MAJOR UPDATE (not minor tweak)
   - Stage 1 version was placeholder (assumptions)
   - Stage 4 version is concrete (based on actual specs)
...
5. ⚠️ Test plan will update AGAIN in Stage 5e
   - Stage 4: Based on SPECS (planned implementation)
   - Stage 5e: Based on ACTUAL CODE (real implementation)
   - Mark clearly as "Stage 4 version - will update in Stage 5e"
...
7. ⚠️ Mark update in Update Log
   - Document what changed from Stage 1 to Stage 4
   - Why changes were made

8. ⚠️ Update epic EPIC_README.md Epic Completion Checklist
   - Mark Stage 4 items complete
```
...
**Verify BEFORE starting Stage 4:**
...
□ epic_smoke_test_plan.md exists (created in Stage 1)
...
- ❌ STOP - Do NOT proceed with Stage 4
...
Step 5: Update epic_smoke_test_plan.md
   ...
   ├─ Mark as "Stage 4 version"
   └─ Update Update Log

Step 6: Mark Stage 4 Complete
...
Read the file created in Stage 1.

**It should have:**
- Version marker: "INITIAL (Stage 1)"
...
- Update Log (one entry from Stage 1)
...
- What are the ACTUAL integration points? (discovered in Stage 3)
...
- [ ] Integration Points → Add from Stage 3 findings
...
From Stage 3 sanity check and feature specs:
...
- Created: {Stage 1 date}
- Last Updated: {Stage 4 date}
...
|------|-------|--------------|-----|
| {Stage 1 date} | Stage 1 | Initial plan created | Epic planning - assumptions only |
| {Stage 4 date} | Stage 4 | **MAJOR UPDATE** | Based on feature specs (Stages 2-3) |
...
Following `stages/stage_5/round1_todo_creation.md` (Round 1) to create comprehensive implementation plan with 28 verification iterations across 3 rounds.
```
...
**Stage 4 is complete when ALL of these are true:**
...
□ epic_smoke_test_plan.md updated with:
  ...
  - High-Level Test Categories: Guidance for Stage 5e updates
  - Update Log: Stage 4 entry documenting changes
...
□ Epic EPIC_README.md updated:
  - Epic Completion Checklist: Stage 4 items checked
  - Agent Status: Phase = IMPLEMENTATION, ready for Stage 5a
...
❌ "Stage 1 test plan looks good enough"
   ✅ STOP - Stage 1 was placeholder, Stage 4 needs major update
...
❌ "Test commands can be TBD, we'll figure it out in Stage 5"
   ✅ STOP - Stage 4 should have actual commands from specs
...
❌ "Let me start implementing features"
   ✅ STOP - Stage 5a (TODO creation) comes BEFORE implementation

❌ "This test plan is final, won't change"
   ✅ STOP - Plan will update in Stage 5e after each feature
```
...
6. ⚡ After marking Stage 4 complete (Step 6)

---

## Prerequisites for Stage 5a

**Before transitioning to Stage 5a, verify:**

□ Stage 4 completion criteria ALL met
...
**Remember:** Use the phase transition prompt from `prompts_reference_v2.md` when starting Stage 5a.
