# S6: Epic-Level Final QC Guide (ROUTER)

🚨 **IMPORTANT: This guide has been split into focused sub-stages**

**This is a routing guide.** The complete S6 workflow is now split across four focused guides:
...
|---------------|---------------|---------------|
| Starting S6 | `stages/s6/s6_p1_epic_smoke_testing.md` | 60-90 min |
| S6.P1: Epic Smoke Testing | `stages/s6/s6_p1_epic_smoke_testing.md` | 45-75 min |
| S6.P2: QC Rounds | `stages/s6/s6_p2_epic_qc_rounds.md` | 2-3 hours |
| S6.P3: User Testing | `stages/s6/s6_p3_user_testing.md` | Variable |
| S6.P4: Epic Final Review | `stages/s6/s6_p4_epic_final_review.md` | 1.5-2 hours |
...
**Exit Condition:** S6 is complete when epic smoke testing passed, all 3 QC rounds passed, user testing passed (no bugs found), epic PR review passed (all 11 categories), and all issues resolved (no pending bug fixes)
...
**What it covers:**
- **Step 1:** Pre-QC Verification (verify all features at S5e, no pending bug fixes)
...
**When complete:** Transition to S6.P2
...
**When complete:** Transition to S6.P3 (User Testing)
...
**Read:** `stages/s6/s6_p3_user_testing.md`
...
**When complete:** Transition to S6.P4 (Epic Final Review)
...
**CRITICAL:** If ANY issues are found during S6, you MUST:

1. **Create bug fixes** using bug fix workflow (S2 → 5a → 5b → 5c)
2. **RESTART S6 from STAGE_6a** (cannot partially continue)
3. **Re-run ALL steps:**
...
**Cannot proceed to S7 without passing S6 restart.**
...
```
┌─────────────────────────────────────────────────────────────┐
│ CRITICAL RULES - Apply to ALL S6 sub-stages           │
└─────────────────────────────────────────────────────────────┘

1. ⚠️ ALWAYS use EVOLVED epic_smoke_test_plan.md (not original from S1)
   - Plan evolved through S4 and S5e
...
3. ⚠️ Epic-level validation focuses on INTEGRATION
   - Feature-level validation done in S5c
...
5. ⚠️ If ANY issues found → create bug fix → RESTART S6
...
8. ⚠️ 100% test pass rate required throughout S6
...
### If you're starting S6:

**READ:** `stages/s6/epic_smoke_testing.md`
...
### If you're resuming mid-S6:

**Check EPIC_README.md Agent Status** to see current step:

```markdown
**Current Stage:** S6 - Epic Final QC
**Current Step:** Step {N} - {Description}
```

**Then read the appropriate guide:**
- **Step 1 or 2:** Read stages/s6/epic_smoke_testing.md
- **Step 3, 4, or 5:** Read stages/s6/epic_qc_rounds.md
- **Step 6:** Read stages/s6/user_testing.md
- **Step 7, 8, or 9:** Read stages/s6/epic_final_review.md
...
- **READ:** `stages/s6/epic_qc_rounds.md` (full guide)
...
□ **Documentation:**
  - epic_lessons_learned.md updated with S6 insights (all sub-stages)
  - EPIC_README.md Epic Progress Tracker shows S6 complete
  - EPIC_README.md Agent Status shows S6c complete

□ **Bug Fixes (if any):**
  - All bug fixes created and completed (S5c)
  - S6 RESTARTED after bug fixes (from STAGE_6a)
...
**Q: What if I find issues during S6?**
A: Create bug fixes using bug fix workflow (S2 → 5a → 5b → 5c), then RESTART S6 from STAGE_6a.

**Q: Can I partially continue S6 after bug fixes?**
A: No. You MUST COMPLETELY RESTART S6 from STAGE_6a (smoke testing) after ANY bug fixes.
...
A: No. You MUST use the EVOLVED epic_smoke_test_plan.md (updated in S4 and S5e). The original plan is outdated.
...
The original guide has been preserved for reference but is deprecated. All S6 work should use the new split guides (6a, 6b, 6c).

---

## Summary

**S6 is now split into four focused guides:**

1. **stages/s6/epic_smoke_testing.md** - Epic Smoke Testing (Steps 1-2)
2. **stages/s6/epic_qc_rounds.md** - Epic QC Rounds (Steps 3-5)
3. **stages/s6/user_testing.md** - User Testing (Step 6)
4. **stages/s6/epic_final_review.md** - Epic Final Review (Steps 7-9)
...
**Start here:** `stages/s6/epic_smoke_testing.md` (unless resuming mid-stage)
...
*End of stages/s6/epic_final_qc.md (ROUTER)*
