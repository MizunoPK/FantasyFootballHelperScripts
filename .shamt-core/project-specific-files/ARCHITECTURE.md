...
> **Note on the root `README.md` / `QUICK_START_GUIDE.md`:** these are partially stale. They reference entry-point scripts that do not exist in the current tree (e.g. `run_simulation.py`, `run_scores_fetcher.py`, `run_game_data_fetcher.py`, `run_draft_order_simulation.py`, `run_draft_order_loop.py`) and an out-of-date test count. This document and the code are the source of truth; the actual entry points are enumerated under **Components and Boundaries** below.
