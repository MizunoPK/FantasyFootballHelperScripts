...
Update History:
  - 2026-08-12: D5.1 probe-based-tier-reachability-guard — recorded the change in kind at the `ConfigManager.py` §"Components and Boundaries" entry: the unit adds a load-time tier-reachability guard, so config load is now fail-closed and a scoring ladder that cannot reach all five tier labels raises `ValueError` during `ConfigManager` construction, hard-failing startup of the League Helper and both simulation engines. Raised at `/du5-review` as a Documentation Impact (Required): the existing "loads and validates" clause did not convey a new load-time failure mode with application-wide blast radius. Applied at `/du6-polish`, following the D3.2 / D3.5 precedent below. No component boundary changed — the guard is a check inside an existing class, adds no network call and no new entry point, so §"Data fetchers / compilers", the network-boundary statement and the §"Data Stores" table remain true as written (slug: D5.1-probe-based-tier-reachability-guard)
  - 2026-08-10: D3.5 top-level-script-doc-registration-reconcile-three-surfaces — registered the D2.3 seed-sweep harness `run_accuracy_seed_sweep.py` in §"Project Structure", the last of the three project-structure surfaces (`README.md`'s tree, `QUICK_START_GUIDE.md`'s "All Scripts" table, this tree), none of which it appeared in; the measured 10 × 3 registration matrix goes from 27/30 to 30/30. Also replaced the §Overview note on the root `README.md` / `QUICK_START_GUIDE.md`, whose two factual clauses (five named entry-point scripts said not to exist, and an out-of-date test count) were measured false this session, with a scoped note that preserves this document's source-of-truth precedence and records the reconciliation — applied under a user-approved amendment to the unit's diff manifest, since ARCHITECTURE prose beyond the tree entry sits outside it (the D3.2 entry below is the precedent). A third clause, retained verbatim in the first pass, pointed readers at §"Components and Boundaries" as the entry-point enumeration; raised at `/du5-review` as a CONCERN (that section names 6 of the 10 tracked top-level scripts, this unit's own subject among the absent four) and narrowed at `/du6-polish` to point at the §"Project Structure" tree for the complete set. No component boundary changed — the harness orchestrates the existing `run_accuracy_simulation.py` CLI as a subprocess rather than importing `AccuracySimulationManager`, and makes no network call, so §"Data fetchers / compilers" and the network-boundary statement remain true as written (slug: D3.5-top-level-script-doc-registration-reconcile-three-surfaces)
...
> **Note on the root `README.md` / `QUICK_START_GUIDE.md`:** these are user-facing summaries; this document and the code remain the source of truth, the complete set of tracked top-level entry-point scripts is enumerated in the tree under **Project Structure** below, and the five component boundaries they group into are described under **Components and Boundaries**. Their script registries — `README.md`'s project-structure tree and `QUICK_START_GUIDE.md`'s "All Scripts" table, alongside the tree under **Project Structure** below — were reconciled against the tracked top-level `*.py` scripts and verified consistent on 2026-08-10 (D3.5-top-level-script-doc-registration-reconcile-three-surfaces).
...
- `league_helper/util/ConfigManager.py` — loads and validates `league_config.json` (+ week overrides). Validation is **fail-closed at load**: a scoring ladder whose tiers cannot all be reached raises `ValueError` during construction, so a bad config hard-fails startup of the League Helper and both simulation engines rather than scoring silently.
...
---
Validated 2026-08-12 — 1 rounds, 1 adversarial sub-agent confirmed (sha256:a7855d9d774e2c51) (D5.1 /du6-polish — load-time fail-closed config validation)
