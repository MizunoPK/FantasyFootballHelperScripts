...
Update History:
  - 2026-08-12: D6.1 fold-team-rank-populate-into-refresh-team-context — refreshed the whole-suite figure in §"Automated test infrastructure" from **3,516 tests across 154 files** (the D4.3 measurement below, accurate as of that entry) to **3,540 across 155**, measured 2026-08-12 on `unit/D6.1` over the working tree committed as `897e1de0`, with the file count independently re-derived by `find tests -name "test_*.py" | wc -l` → `155`. This unit contributes the whole **`+6 tests / +0 files`** of the *test* drift and **none** of the file drift: pristine `main` runs `2 failed, 3532 passed` (3,534 collected) and this branch runs `2 failed, 3538 passed` (3,540), the `+6` being the unit's own new mutation-discriminating methods (`+5` in the renamed `tests/league_helper/util/test_PlayerManager_team_context.py`, `+1` in the renamed `tests/simulation/win_rate/test_SimulatedLeague_team_context_refresh.py`). The `+1` file drift **predates this branch** — `origin/main` already carries 155 test files (`git ls-tree -r origin/main` re-derived here), D5.1 and D5.2 having landed after the 2026-08-11 measurement — and this unit moves it not at all, consistent with its `**Test Build Plan:** none` (both of its test-file changes are `git mv` renames, so the count is net-zero). **Sourcing note, new with this entry:** the figure is read from the runner's **red** line `FAILURE: 2 of 3540 TESTS DID NOT PASS (3538 passed)` rather than from the `SUCCESS: ALL {N} TESTS PASSED (100%)` line every prior entry quotes, because two pre-existing, unrelated `TestT69TerminatingRunner` tests fail identically on the unmodified tree ("No baseline config folder found") and the runner therefore exits `1`; that sourcing rule is recorded inline in the section itself so a future reader is not left hunting a green line that does not print. Raised at `/du5-review` as a **Documentation Currency CONCERN** — the stated count is a tool-checkable claim this unit's own diff falsifies — and applied at `/du6-polish` under a **user-approved amendment** to the unit's diff manifest, since this path sits outside it (the identical class and handling as the D4.3, D4.2, D4.1, D3.2 and D8.2/D8.3/D8.4 entries below; the amendment is recorded in the unit's `implementation_plan.md` §"Files Touched (manifest)"). The review measured `3,540 / 155` pre-polish and the post-polish re-run confirms the same figure — this polish pass added **no** test method (its Testing SUGGESTION strengthened an existing method in place rather than adding one), so the re-measurement was still performed rather than assumed. The figure supersedes the D4.3 entry's; that entry is left as written, recording the 3,516 measurement as a historical fact. No other section changed, and no template drift (slug: D6.1-fold-team-rank-populate-into-refresh-team-context)
  - 2026-08-11: D4.3 win-rate-e2e-config-isolation — refreshed the whole-suite figure in §"Automated test infrastructure" from **3,506 tests across 154 files** (the D4.2 measurement below, accurate as of that entry) to **3,516 across 154**, measured 2026-08-11 on `unit/D4.3` by a full green run of the declared run-source (`SUCCESS: ALL 3516 TESTS PASSED (100%)`, exit `0`) over the working tree committed as `9c85fe7c`, with the file count independently re-derived by `find tests -name "test_*.py" | wc -l` → `154` (unchanged, consistent with the unit's `**Test Build Plan:** none` — every test this unit adds lands in an existing file). This unit contributes the whole `+10 tests / +0 files` of the drift, in two parts: `+2 / +0` at build — `TestConfigFlagParsing`'s two real-parser methods in `tests/root_scripts/test_run_win_rate_simulation_sweep.py`, which took the suite to 3,508 — and a further `+8 / +0` at `/du6-polish`: six in that same file's new `TestConfigGuards` (the `--promote`/`--config` mutual-exclusion guard and the `--config` path-shape validation, both arms), plus two in `tests/integration/test_simulation_integration.py` (the fixture-constant guard and the `_make_orchestrator` config-root pin). Raised at `/du5-review` as a **Documentation Currency CONCERN** — the stated count is a tool-checkable claim this unit's own diff falsifies — and applied at `/du6-polish` under a **user-approved amendment** to the unit's 12-path diff manifest, since this path sits outside it (the identical class and handling as the D4.2, D4.1, D3.2 and D8.2/D8.3/D8.4 entries below). The review measured `3,508 / 154` pre-polish; the figure written here is the **post-polish** measurement, re-run rather than copied, because the polish pass itself added eight tests. The figure supersedes the D4.2 entry's; that entry is left as written, recording the 3,506 measurement as a historical fact. No other section changed, and no template drift (slug: D4.3-win-rate-e2e-config-isolation)
  - 2026-08-11: D4.2 adp-direction-flip-with-pinned-ladder — refreshed the whole-suite figure in §"Automated test infrastructure" from **3,500 tests across 153 files** (the D4.1 measurement below, accurate as of that entry) to **3,506 across 154**, measured 2026-08-11 on `unit/D4.2` @ `00924c72` by a full green run of the declared run-source (`SUCCESS: ALL 3506 TESTS PASSED (100%)`, exit `0`), with the file count independently re-derived by `find tests -name "test_*.py" | wc -l` → `154`. This unit contributes the whole `+6 tests / +1 file` of the drift, in two parts: `+4 / +1` at build — the new `tests/league_helper/util/test_ConfigManager_adp_ladder.py`, the live-config ladder pin (`test_build_plan.md` Step 1) — and a further `+2 / +0` at `/du6-polish`, the two boundary tests added to that same file closing Copilot PR #95 comment `3758022768` (the computed-threshold and exact-boundary assertions). No other file's count moved: the unit's only other test edit re-pins an existing assertion's value (`tests/root_scripts/test_run_win_rate_simulation_e2e.py`, `10/17` → `8/17`) and adds no method. Raised at `/du5-review` as a **Documentation Currency CONCERN** — the stated count is a tool-checkable claim this unit's own diff falsifies — and applied at `/du6-polish` under a **user-approved amendment** to the unit's diff manifest, since this path sits outside it (the identical class and handling as the D4.1, D3.2 and D8.2/D8.3/D8.4 entries below). The review measured `3,504 / 154` pre-polish; the figure written here is the **post-polish** measurement, re-run rather than copied, because the polish pass itself added the two boundary tests. The figure supersedes the D4.1 entry's; that entry is left as written, recording the 3,500 measurement as a historical fact. No other section changed, and no template drift (slug: D4.2-adp-direction-flip-with-pinned-ladder)
  - 2026-08-10: D4.1 preserve-adp-thresholds-across-promote — refreshed the whole-suite figure in §"Automated test infrastructure" from **3,495 tests across 153 files** (the D3.2 measurement below, still accurate as of that entry) to **3,500 across 153**, measured 2026-08-10 on `unit/D4.1` @ `3fc09c58` by a full green run of the declared run-source (`SUCCESS: ALL 3500 TESTS PASSED (100%)`, exit `0`, recorded in the unit's `testing_results.md`), with the file count independently re-derived by `find tests -name "test_*.py" | wc -l` → `153` (unchanged, consistent with the unit's `**Test Build Plan:** none`). This unit contributes the whole `+5 tests / +0 files` of the drift: five methods appended to the existing `tests/simulation/test_AccuracyResultsManager.py::TestPropagateToConfigs`, corroborated by that module's count moving from the plan's calibrated pre-edit `96 passed` to `101 passed`. Raised at `/du5-review` as a **Documentation Currency CONCERN** — the stated count is a tool-checkable claim this unit's own diff falsifies — and applied at `/du6-polish` under a **user-approved amendment** to the unit's two-row diff manifest, since this path sits outside it (the identical class and handling as the D3.2 and D8.2/D8.3/D8.4 entries below). The figure supersedes the D3.2 entry's figure; that entry is left as written, recording the 3,495 measurement as a historical fact. No other section changed, and no template drift (slug: D4.1-preserve-adp-thresholds-across-promote)
...
- **Status:** **Present.**
- **Runner / command:** `python tests/run_all_tests.py` — the canonical full-suite gate; it discovers every `test_*.py` under `tests/`, runs each through `pytest` with `-m "not live_api"`, and **requires a 100% pass rate** (exit `0` = all passed, `1` = any failure **or** a run that dirtied `data/` — see the data-cleanliness backstop below). The offline suite is **3,540 tests across 155 files** (measured 2026-08-12 on `unit/D6.1` @ `897e1de0` by a full run of this runner; file count re-derived by `find tests -name "test_*.py" | wc -l` → `155`). **The count is sourced from the red line, not the green one:** two pre-existing, unrelated `tests/root_scripts/test_run_accuracy_simulation.py::TestT69TerminatingRunner` tests currently fail ("No baseline config folder found"), so the runner prints `FAILURE: 2 of 3540 TESTS DID NOT PASS (3538 passed)` and exits `1` — the `SUCCESS: ALL {N} TESTS PASSED (100%)` line the earlier Update History entries quote is unavailable while those two stand. Read the total from that line's `{total}` (`3540`), i.e. `3538 passed + 2 failed`. The pre-commit wrapper `python run_pre_commit_validation.py` calls the same runner.
