...
class ErrorHandler:
    ...
    def __init__(self, component_name: str, enable_detailed_logging: bool = True):
        ...
        self.logger = setup_module_logging(f"{__name__}.{component_name}")
    ...
    async def retry_async(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        for attempt in range(self.max_attempts):
            try:
                ...
                if attempt < self.max_attempts - 1:  # Don't delay after last attempt
                    delay = self.calculate_delay(attempt)
                    logger.debug(f"Async retry attempt {attempt + 1} failed, waiting {delay:.2f}s: {e}")
                    await asyncio.sleep(delay)
                else:
                    logger.error(f"All {self.max_attempts} async retry attempts failed: {e}")
