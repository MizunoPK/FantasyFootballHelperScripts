...
already-committed discrimination evidence from
[`_internal/win_rate_similarity_investigation.md`](../../_internal/win_rate_similarity_investigation.md)
(Experiments 1 / 4 / 5 / 6, validated 2026-06-26) into a durable before/after. **No fresh
simulation run** — every number below is transcribed from that committed investigation and its raw
JSON under `_internal/data/`.
...
(`--naive-opponents`, retained from T19/T28: "legacy naive-opponent composition — 1 DraftHelperTeam
+ 9 SimulatedOpponents — instead of the default self-play field; reproduces the prior ~0.84
baseline.")

**What it could measure.** Taking the identical weak-corner vs strong-corner param pair (opposite
corners of the 7-D swept param space) and evaluating each at **1,360 games** (10× the operational
default), the investigation's **Exp 4** measured:
...
*Source: Exp 4, `_internal/data/sims_scaling_results.json`.*

**Why it "could not discriminate" at operational sims.** That +0.026 gap only surfaces at 10×
games. At the **operational 136-game** default (Exp 1), the same pair is **within the evaluation
...
*Source: Exp 1, `_internal/data/noise_results.json` (within-config σ ≈ 0.031; between-config store
spread 0.023).*
...
*Source: Exp 6, `_internal/data/amplification_results.json`.*
...
| pooled two-proportion z | **≈ 3.4** | **≈ 5.0** |
| games / eval | 1,360 | 1,360 |
| operational regime (136 games) | **within noise** (σ ≈ 0.031 ≥ 0.023 spread) — **could not discriminate** | headroom regime (~0.48 baseline) — **discriminates** |
...
`_internal/data/selfplay_ceiling_results.json`.*
...
- **#2 — discrimination** ("the sweep reliably separates two configs whose true win rate differs by
  ...
  make"): **CLOSED** — Exp 6's **+0.095 gap at z ≈ 5.0** clears the ≥ 0.05 bar, a separation the
  naive sweep (Exp 4 / Exp 1, within-noise at operational sims) could not make.
- **#7 — documented before/after** ("a documented before/after demonstrates the sweep discriminating
  … where it previously could not"): **CLOSED** — this document.
...
Exp 1 / 4 / 5 / 6) and its raw JSON under `_internal/data/`. Document-only — no simulation run, no
code or test change, no live-config write. T37, epic T17 / feature T24.*
